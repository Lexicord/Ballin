...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'baller':
				dialogue = CoolUtil.coolTextFile(Paths.txt('baller/Dialogue'));
			case 'balls deep':
				dialogue = CoolUtil.coolTextFile(Paths.txt('balls deep/Dialogue'));
			case 'spherical':
				dialogue = CoolUtil.coolTextFile(Paths.txt('spherical/Dialogue'));
		}
			...
			case 'alley':
				{
						defaultCamZoom = 0.65;
						curStage = 'alley';
						var bg:FlxSprite = new FlxSprite(0,0).loadGraphic(Paths.image('ball/Bg'));
						bg.antialiasing = true;
						bg.screenCenter();
	...
	*/
	...
	override function stepHit()
	{
		...
		#end
		if (curSong.toLowerCase() == 'balls deep' && curStep == 800)
			{
					dad.animation.stop(); //fix for the animations looking weird
					boyfriend.animation.stop();

					dad.playAnim('Special', true);
					boyfriend.playAnim('TakeDamage', true);
				new FlxTimer().start(0.2, function(tmr:FlxTimer)
					{
						FlxG.sound.play(Paths.sound('crash'));
						camHUD.shake(0.1, 0.2);
		...
		if (FlxG.random.bool(3) && PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && curStep < 800)
			{
				if (curSong.toLowerCase() == 'spherical'){
					dad.animation.stop(); //fix for the animations looking weird
					boyfriend.animation.stop();
				...
				new FlxTimer().start(0.2, function(tmr:FlxTimer)
					{
						FlxG.sound.play(Paths.sound('crash'));
						camHUD.shake(0.085, 0.2);
						FlxG.camera.shake(0.085, 0.2);
						var healthtochange:Float = health;
						healthtochange -= 0.75;
						if (healthtochange <= 0){
							trace('cringe');
	...
	override function beatHit()
	{
		...
		if (curSong == 'balls deep' && curBeat % 4 == 0)
			{
				if (camHUD.angle == -60){ //idk i thought it looked cool 
					FlxTween.tween(camHUD, {angle: 60},1, {ease: FlxEase.elasticOut});
				}
				else{
					FlxTween.tween(camHUD, {angle: -60},1, {ease: FlxEase.elasticOut});
				}
		...
		{
			if (curSong.toLowerCase() == 'balls deep'){
